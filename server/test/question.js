...
export default (test) => {
  ...
  const sharedInputOther = {text: 'Maya?', expirationDate: moment().add(2, 'days').toDate()};
  ...
  test('POST /api/question - should not create new question with malformed date', (t) => {
    const input = {text: 'Any question?', expirationDate: 'not a date'};
    request(app)
      .post('/api/question')
      .set('x-access-token', app.get('token'))
      .send(input)
  ...
  test('POST /api/question/:id/answer - answer existing question', (t) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        t.error(err, 'No error');
        t.equal(actualBody.answers.length, 1, answer);
  ...
  test('GET /api/question - get latest questions', (t) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        t.ok(moment(actualBody[0].creationDate).isValid(), 'Creation Date is valid');
        t.ok(
        ...
        t.equal(actualBody[1].owner.id, app.get('user').id, 'Question belongs to correct user');
        t.ok(moment(actualBody[1].creationDate).isValid(), 'Creation Date is valid');
        t.ok(
  ...
  test('POST /api/question/:id - should not update question with invalid date', (t) => {
    request(app)
      .post(`/api/question/${app.get('question').id}`)
      .set('x-access-token', app.get('token'))
      .send({text: 'question?', expirationDate: 'not a valid date'})
      .expect(400)
      .expect('Content-Type', /json/)
      .end((err, res) => {
        const expectedBody = {error: 'Date should be valid ISO Date!'};
  ...
  test('POST /api/question/:id - should update question with a new text', (t) => {
    request(app)
      .post(`/api/question/${app.get('question').id}`)
      .set('x-access-token', app.get('token'))
  ...
  test('DELETE /api/question/:id - should not delete question with different owner', (t) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        t.error(err, 'No error');
        t.deepEqual(actualBody, expectedBody, 'Retrieve cannot delete other question');
        t.end();
      });
  });
  ...
  test('DELETE /api/question/:id - should delete question', (t) => {
    request(app)
      ...
      .end((err) => {
        ...
        t.test('GET /api/question/:id - should fail to get dleted question', (st) => {
          request(app)
            .get(`/api/question/${app.get('question').id}`)
            .set('x-access-token', app.get('token'))
            .expect(400)
