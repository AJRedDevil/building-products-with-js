...
test('# Register page wrapper', () => {
  const store = mockStore({auth: {redirectToLogin: true}});
...
test('# Register page', () => {
  ...
  // mount
  const app = mount(<MemoryRouter initialEntries={[{path: '/', key: 'testKey'}]}>{component}</MemoryRouter>);
  // set new login, pass and passRepeat
